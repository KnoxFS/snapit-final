...
const Features = () => {
  ...
  return (
    <section
      className='sm:w-[90%] md:max-w-[1280px] mx-auto md:mt-[140px] mt-[80px]'
      id='features'>
        ...
        <div className='rounded-md p-6 text-white h-full text-center'>
          ...
          <div className='w-full flex justify-center'>
            <ul className='flex flex-wrap pt-3 justify-center items-center md:w-6/12'>
              {uses.map(use => (
                <li
                  className='my-2 mr-3 rounded-full border border-primary px-3 text-primary py-1 text-sm shadow-md hover:-translate-y-1 hover:shadow-lg hover:bg-primary hover:text-white select-none transition xs:'
                  key={use}>
                  {use}
                </li>
              ))}
