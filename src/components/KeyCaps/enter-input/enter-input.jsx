...
const EnterInput = ({ value, setValue }) => {
  ...
  return (
    ...
    >
      <input
        className="block w-full rounded-xl border-gray-300 px-4 py-5 text-lg leading-none shadow-sm placeholder:italic sm:w-3/5 lg:w-96"
        name="keyboard"
        type="text"
        placeholder={
          isPressMode
      ...
      <div className="flex justify-center space-x-5 sm:flex-col sm:space-x-0 sm:space-y-3 lg:flex-row lg:items-center lg:space-y-0 lg:space-x-5">
        {types.map(({ id, title }) => (
          <div key={id} className="flex items-center">
            <input
              className="border-gray-2 checked:border-black-2 checked:hover:border-black-2 checked:focus:border-black-2 peer h-5 w-5 border bg-transparent text-transparent transition-colors duration-150 focus:ring-0 focus:ring-offset-0"
              id={id}
              name="enter type"
              type="radio"
            ...
            <label
              htmlFor={id}
              className="block pl-2 font-medium leading-none text-white transition-colors duration-150 md:pl-3 md:text-lg"
            >
              {title}
            </label>
          </div>
